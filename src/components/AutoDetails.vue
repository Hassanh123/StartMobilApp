...
<div class="mb-6 flex justify-center gap-4 flex-wrap">
  <input
    ...
    class="w-full max-w-sm sm:max-w-md md:max-w-xl px-6 py-4 md:px-7 md:py-5 text-lg md:text-xl border border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition duration-150"
  ...
  <select
    id="sorteren"
    v-model="sortering"
    class="px-6 py-4 md:px-7 md:py-5 text-lg md:text-xl border border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition duration-150"
 ...
 <!-- Auto kaarten -->
<div v-else class="flex flex-wrap justify-center gap-8 px-4">
  <div
    v-for="(auto) in gefilterdeAutos"
    :key="auto.id"
    class="border border-gray-200 rounded-2xl p-5 w-full sm:w-96 md:w-[28rem] bg-white shadow-md hover:shadow-xl transition-shadow duration-300"
  >
    ...
    <img
      v-if="auto.foto"
      :src="`/images/cars/mobile/${auto.foto}`"
      :srcset="`
      ...
      :title="`Foto van ${auto.merk} ${auto.model}`"
...
<div class="space-y-1 text-lg sm:text-xl md:text-2xl text-gray-700">
